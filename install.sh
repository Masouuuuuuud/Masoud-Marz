...
is_running() { docker ps | grep -q "masoud"; }

create_directories() {
    mkdir -p "$INSTALL_DIR" "${INSTALL_DIR}/assets"
}
...
show_xray_versions() {
    ...
    curl -s "$GITHUB_API" | jq -r '.[0:40] | .[] | .tag_name' | nl
}

select_xray_version() {
    show_xray_versions
    local choice
    read -p "Select Xray version (1-40): " choice
    local selected_version=$(curl -s "$GITHUB_API" | jq -r ".[0:40] | .[$((choice-1))] | .tag_name")
...
download_xray_core() {
    ...
    wget -q --show-progress "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/latest/download/geoip.dat" -O "${INSTALL_DIR}/assets/geoip.dat"
    wget -q --show-progress "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/latest/download/geosite.dat" -O "${INSTALL_DIR}/assets/geosite.dat"
    wget -q --show-progress "https://github.com/bootmortis/iran-hosted-domains/releases/latest/download/iran.dat" -O "${INSTALL_DIR}/assets/iran.dat"
...
services:
  masoud:
    ...
    environment:
      ...
      SSL_CLIENT_CERT_FILE: "/var/lib/masoud/client.pem"
...
show_status() {
    ...
    if is_running; then
        success "Status: Up and Running [uptime: $(docker ps --filter "name=masoud_1" --format "{{.Status}}")]"        
    else
...
main() {
    ...
    case "$1" in
        install)         install_masoud ;;
        uninstall)       uninstall_masoud ;;
        update)          update_masoud ;;
